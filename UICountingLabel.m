...
- (void)setTextValue:(CGFloat)value {
    if (self.attributedFormatBlock != nil) {
        ...
        if (self.positiveFormat) {
            NSNumberFormatter *formatter = [[NSNumberFormatter alloc] init];
            formatter.positiveFormat = self.positiveFormat; // 正数格式
            self.text = [formatter stringFromNumber:@(value)];
        } else {
            BOOL isIntegerValue = [self.format rangeOfString:@"%(.*)d" options:NSRegularExpressionSearch].location != NSNotFound || [self.format rangeOfString:@"%(.*)i"].location != NSNotFound;
            if (isIntegerValue) {
